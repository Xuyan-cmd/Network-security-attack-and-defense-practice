...
- 2023年7月18日，重复试了很多次都不能正常访问到最内层的主机后，无意间发现最内层的主机的容器在开启后就退出了，本来是通过`docker ps`来查看运行中的容器，发现并没有`vulshare/nginx-php-flag`的容器，通过`docker ps -a`查看到了`Exit(1)`的状态，并尝试通过`docker logs`来查看引发错误的日志，结果也看不出什么头绪，单看ping的报错的话，像是网卡配置问题的样子🤔
